...
void Server::Join(std::vector<std::string>& params, Client& cli)
{
    ...
    if (params.empty())
    {
        Utils::write_message(cli._cliFd, ERR_NEEDMOREPARAMS(cli._nick, "JOIN"));
        return;
    }
    ...
    for (size_t i = 0; i < channels.size(); ++i)
    {
        ...
        if (chan[0] != '#' && chan[0] != '&')
        {
            Utils::write_message(cli._cliFd, ERR_INVCHANNAME);
            return;
        }

        if (client_is_in_there(cli, chan) == 0)
        {
            if (!cli._nick.empty())
            {
                int isThere = 0;
                for (chanIt it = _channels.begin(); it != _channels.end(); ++it)
                {
                    if (it->_name == chan)
                    {
                        if (it->_key == key)
                        {
                            ...
                            {
                                Utils::write_message(cli._cliFd, ERR_CHANNELISFULL(cli._nick, chan));
                                return;
                            }
                            it->_channelClients.push_back(cli);
                            it->_opNick = it->_channelClients[0]._nick;
                            Utils::write_message(cli._cliFd, RPL_JOIN(cli._nick, cli._ip, chan));
                            std::cout << PURPLE << "Client " << cli._nick << " has entered \'" << chan << "\'" << RESET << std::endl;
                            show_right_gui(cli, *it);
                        }
                        else
                        {
                            Utils::write_message(cli._cliFd, ERR_BADCHANNELKEY(cli._nick, chan));
                        }
                        isThere = 1;
                        break;
                    }
                ...
                if (isThere == 0)
                {
                    ...
                    show_right_gui(cli, tmp);
                }
            }
            else
            {
                Utils::write_message(cli._cliFd, "Set your nick before!\r\n");
            }
        }
        else
        {
            Utils::write_message(cli._cliFd, "You are already in this channel\r\n");
        }
    }
}
