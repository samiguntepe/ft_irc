...
void Server::Kick(std::vector<std::string>& params, Client& cli)
{
    ...
    if (params.size() < 2) {
        ...
        Utils::write_message(cli._cliFd, ERR_CHANOPKICK(cli._nick, params[0]));
        return ;
    }
    else
    {
            ...
            if (it->_name == params[0])
            {
                ...
                for (cliIt it2 = it->_channelClients.begin(); it2 != it->_channelClients.end(); ++it2) {
                    ...
                    if (it2->_nick == params[1]) {
                        ...
                        if (params.size() == 3)
                            msg = params[2];
                        Utils::write_message(it2->_cliFd, RPL_KICK(it->_opNick, params[0], params[1], msg));
        ...
        if (flag == 0)
            ...
            Utils::write_message(cli._cliFd, ": 431 " + cli._nick + " " + params[0] + " : " +  params[1] + " not in this channel\r\n");
    }
}
