...
void Channel::setUpModeChannel(Channel* channel, Client* client, string& mode, string& modeParams)
{
	...
	if (mode == "+k" && (client->isOperator() == true))
	{
		...
		channel->broadcastMessage("MODE " + channel->getchannel_name() + " +k " + client->getNickName());
	}
	else if (mode == "+l" && (client->isOperator() == true))
	{
		channel->setUserLimit(atoi(modeParams.c_str()));
		channel->broadcastMessage("MODE " + channel->getchannel_name() + " +l " + client->getNickName());
	}
	else if (mode == "+o" && (client->isOperator() == true))
	{
		...
		target_client->setOperator(true);
		channel->broadcastMessage("MODE " + channel->getchannel_name() + " +o " + target_client->getNickName());
	}
	else if (mode == "+m" && client->isOperator() == true)
	{
		channel->setModerated(true);
		channel->broadcastMessage("MODE " + channel->getchannel_name() + " +m " + client->getNickName());
	}
...
void Channel::setLowModeChannel(Channel* channel, Client* client, string& mode, string& modeParams)
{
	...
	else if (mode == "-o" && (client->isOperator() == true))
	{
		...
		if (target_client->getNickName() != channel->getChannelOwner()->getNickName())
		{
			target_client->setOperator(false);
			channel->broadcastMessage("MODE " + channel->getchannel_name() + " -o " + target_client->getNickName());
		}
	}
	else if (mode == "-m" && client->isOperator() == true)
